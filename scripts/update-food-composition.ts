...
function parseIngredients(ingredientsRaw: string): string[] {
  ...
  for (const part of parts) {
    ...
    if (cleaned.includes("fruktooligosacharydy") || cleaned.includes("fos")) {
      ...
      cleaned.includes("mannanoligosacharydy") ||
      cleaned.includes("mannan-oligosacharydy") ||
      cleaned.includes("mos")
    ) {
...
async function updateFoodComposition(foodName: string, ingredientsRaw: string) {
  ...
  try {
    ...
    if (foodError || !food) {
      const { data: allFoods } = await supabase.from("foods").select("id, name").ilike("name", `%${foodName}%`);
    ...
    for (const ingredient of allIngredients) {
      ...
      const mappingsToAdd: { ingredient_id: number; allergen_id: number }[] = [];
      ...
      if (mappingsToAdd.length > 0) {
        await supabase.from("ingredient_allergens").upsert(mappingsToAdd, { onConflict: "ingredient_id,allergen_id" });
    ...
    const { error: deleteError } = await supabase.from("food_ingredients").delete().eq("food_id", food.id);
    ...
    const { error: insertFoodIngError } = await supabase.from("food_ingredients").insert(foodIngredients);
