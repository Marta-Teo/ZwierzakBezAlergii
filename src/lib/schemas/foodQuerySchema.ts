...
export const FoodQuerySchema = z.object({
  ...
  brandId: z.string().regex(/^\d+$/, "Brand ID musi być liczbą").transform(Number).optional(),

  sizeTypeId: z.string().regex(/^\d+$/, "Size Type ID musi być liczbą").transform(Number).optional(),

  ageCategoryId: z.string().regex(/^\d+$/, "Age Category ID musi być liczbą").transform(Number).optional(),
  ...
  search: z.string().trim().min(1, "Zapytanie wyszukiwania nie może być puste").optional(),
